...
const TicTacToe = () => {
  ...
  const winningDiagonals = [[0,4,8], [2,4,6]];
  const winningCombinations = [...winningColumns, ...winningRows, ...winningDiagonals];

  const changePlayer = () => (whoseTurnIsIt === 'X') ? setWhoseTurnIsIt('O') : setWhoseTurnIsIt('X');

  const checkWin = (player) => {
    const hasWinner = winningCombinations.some(arrayOfWinningPositions => {
      const winningCheckArray = arrayOfWinningPositions.map(item => board[item] === player);
    ...
    const boardStillHasMovesLeft = board.some(item => item === CELL_NOT_FILLED);
